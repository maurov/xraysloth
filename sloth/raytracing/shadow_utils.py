...
_logger = getLogger("shadow_utils")
...
def rotate_rays(rays, angle, axis):
    ...
    for i in range(len(tstart)):
        ...
        rays[:, newtoroti[0]] = (
            a1[:, newtoroti[0]] * costh + a1[:, newtoroti[1]] * sinth
        )
        rays[:, newtoroti[1]] = (
            -a1[:, newtoroti[0]] * sinth + a1[:, newtoroti[1]] * costh
        )
...
def plot_footprint(return_tkt=False, **h2args):
    ...
    _popnolost = h2args.pop("nolost", None)  # noqa
    _popref = h2args.pop("ref", None)  # noqa
    _poptitle = h2args.pop("title", None)  # noqa
    try:
        tkt = Shadow.ShadowTools.plotxy(
            "mirr.01", 2, 1, nolost=2, ref=0, title="Footprint", **h2args
        )
    except Exception:
        _logger.error('probably "mirr.XX" file does not exist!')
        tkt = 0
    if return_tkt:
        return tkt
...
def plot_image(beam, return_tkt=False, **h2args):
    ...
    _popnolost = h2args.pop("nolost", None)  # noqa
    _popref = h2args.pop("ref", None)  # noqa
    _popcalcw = h2args.pop("calculate_widths", None)  # noqa
    tkt = Shadow.ShadowTools.plotxy(
        beam, 1, 3, ref=23, nolost=1, calculate_widths=2, **h2args
    )
    if return_tkt:
        return tkt
...
def get_origin_scale(h2tkt):
    ...
    origin = (xmin, ymin)
    scale = (
        abs((xmax - xmin) / h2tkt["nbins_h"]),
        abs((ymax - ymin) / h2tkt["nbins_v"]),
    )
    return (origin, scale)
...
def histo2_normed(
    ...
    if ref == 1:
        print(
            "Shadow.Beam.histo2: Warning: weighting with column 1 (X) [not with\
    ...
    if xrange is None:
        xrange = beam.get_good_range(col_h, nolost=nolost)
    if yrange is None:
        yrange = beam.get_good_range(col_v, nolost=nolost)
    ...
    if calculate_widths > 0:
        ...
        if h[tt].size > 1:
            ...
            ticket["fwhm_coordinates_h"] = (
                ticket["bin_h_center"][tt[0][0]],
                ticket["bin_h_center"][tt[0][-1]],
            )
        else:
            ticket["fwhm_h"] = None
        ...
        if h[tt].size > 1:
            ...
            ticket["fwhm_coordinates_v"] = (
                ticket["bin_v_center"][tt[0][0]],
                ticket["bin_v_center"][tt[0][-1]],
            )
        else:
            ticket["fwhm_v"] = None
