...
_LOGGER = getLogger("crystal_box", level="INFO")
...
try:
    ...
    try:
        ...
        if "linux" in _platform:
            ...
            _LOGGER.error("cannot set DIFFPAT_EXEC")
            # sys.exit(1)
    except KeyError:
        _LOGGER.error("$DIFFPAT_EXEC or $XOP_HOME environmental variables not set!")
        # sys.exit(1)

# ../../xop/data
_pardir = os.path.realpath(
...
class XCrystalBox(object):
    ...
    def check_opts(self):
        ...
        if self.opts["bragg_file"] is None:
            ...
            if not os.path.exists(bf_in_datadir):
                raise NameError(
                    "{} not found in {}".format(self.opts["bragg_file"], DATA_DIR)
                )
            else:
    ...
    def load_refl(
        self, fname="diff_pat.dat", pol="s", kind="cubic", fill_value=0.0, plot=False
    ):
