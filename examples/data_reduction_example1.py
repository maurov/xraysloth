...
This is an example of data reduction performed by the author
(M. Rovezzi). The workflow presented here is applied to a set of
peak-like 1D scans (XY) collected at the beamline ID26 of ESRF in SPEC
format (data file: 'peakfit_tests_real.dat'). The steps followed in
the data reduction are:
...
def eval_scan(fndat, scan, infodict, counter=1, signal='det_dtc',
    ...
    infodict : dictionary describing scan / experiment / sample / whatever
               ...
               *example of common keys*:
               ...
               'stage'   : int, spectrometer position of the crystal analyzer (between 1 and 5)
    ...
    """
    infodict.update({'date'    : '{0}_{1}_{2}_{3}_{4}_{5}'.format(*time.localtime()),
                     'fndat'   : fndat,
                     'scan'    : scan,
                     'norm'    : norm,
                     'counter' : counter,
    ...
    x, y_sec, motors, infos = t.get_scan(scan, cntx=counter,
                                         csig=seconds, cmon=None,
                                         csec=None, norm=None)
    x, y_mon, motors, infos = t.get_scan(scan, cntx=counter,
                                         csig=monitor, cmon=None,
                                         csec=None, norm=None)
    x, y_sig, motors, infos = t.get_scan(scan, cntx=counter,
                                         csig=signal, cmon=None,
                                         csec=None, norm=None)
    if norm == 'mon':
        y = y_sig/y_mon
    ...
    # fit x,y
    try:
        fit, pw = fit_splitpvoigt(x, y, dy=True, bkg='Constant',
                                  plot=showFit, show_res=showFit)
        fit.xrel = x - fit.resdict['cfwhm']
        print('=> OK PEAK FIT [{0}] <='.format(label))
    except:
        print('=> ERROR PEAK FIT [{0}] <='.format(label))
        fit.resdict = {'area' : np.nan,
                       'cfwhm' : np.nan,
                       'fwhm' : np.nan,
                       'height' : np.nan,
                       'position' : np.nan}
    ...
    infodict.update({'y_area' : np.trapz(y),
                     'y_max'  : np.max(y),
                     'y_imax' : imax(y),
                     'y_min'  : np.min(y),
                     'y_imin' : imin(y)})
...
def eval_loop(scntype, fns, scans, labs, marks, rads, poss, flags,
              framert=125., axsep=120., theta0=75., fnout=None,
              owrt=True, retall=False, show_results=False, **kws):
    ...
    for fn, scan, lab, mark, rad, pos, flag in zip(fns, scans, labs,
                                                   marks, rads, poss, flags):
        if flag == 0: continue
        idict = {'label' : lab,
                 'mark'  : mark,
                 'radius': rad,
    ...
    if fnout is not None:
        ...
        for lcol, ldat, linf in zip(lcols, ldats, linfs):
            ...
            title = '{0}_R{1:.0f}_P{2}_M{3}_{4}'.format(linf['label'],
                                                        linf['radius'],
                                                        linf['stage'],
                                                        linf['mark'],
                                                        linf['scntype'])
    ...
    if retall: return lcols, ldats, linfs
...
def eval_show_results(linfdict, norm_to_first=False):
    ...
    """
    _tmpl_head = '{0:^15} & {1:^5} &  {2:^5} & {3:^8} & {4:^7} & {5:^8} & {6:^8} & {7:^8}'
    _tmpl_res  = '{0:^15} & {1:^5.0f} & {2:^5.0f} & {3:^8.2f} & {4:^7.3f} & {5:^8.2f} & {6:^8.4f} & {7:^8.4f}'
    print('=> RESULTS <=')
    print(_tmpl_head.format('label', 'pos', 'mark', 'radius', 'fwhm', 'cfwhm', 'height', 'area'))
    for idic, dic in enumerate(linfdict):
        ...
        if norm_to_first:
            ...
            height0 = 1.
            area0 = 1.
        print(_tmpl_res.format(dic['label'], dic['stage'], dic['mark'], dic['radius'], dic['fwhm'], dic['cfwhm'], dic['height']/height0, dic['area']/area0))
...
if __name__ == '__main__':
    ...
    if 1:
        ...
        labs   = ['T1', 'T2',  'T3',  'T4' ]
        marks  = [1,     1,     1,    1    ]     
        rads   = [1000., 1000., 500., 500. ] 
        poss   = [1,     2,     3,    4    ]
        flags  = [1,     1,     1,    1    ]

        lcols_el, ldats_el, linfs_el = eval_loop('elastic', fns,
                                                 escans, labs,
                                                 marks, rads,
                                                 poss, flags,
                                                 fnout=fnout_el,
